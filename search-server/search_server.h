...
public:
			...
			stop_words_(MakeUniqueNonEmptyStrings(stop_words)) // Extract non-empty stop words
	{
		if (!all_of(stop_words_.begin(), stop_words_.end(), IsValidWord)) {
			throw std::invalid_argument(
					static_cast<std::string>("Some of stop words are invalid"));
		}
	}
	...
	explicit SearchServer(std::string_view stop_words_text) :
			SearchServer(
					SplitIntoWords(static_cast<std::string>(stop_words_text))) // Invoke delegating constructor
	// from string container
	{
	}
...
std::vector<Document> SearchServer::FindTopDocuments(
	...
	} else if (std::is_same_v<std::decay_t<ExecutionPolicy>,
...
std::vector<Document> SearchServer::FindAllDocuments(
	...
	for (const std::string &word : query.minus_words) {
		...
		for (const auto [document_id, _] : word_to_document_freqs_.at(word)) {
			async([&document_to_relevance, &document_id] {
				document_to_relevance.erase(document_id);
			});
		}
	}
