...
public:
	...
	struct Access {
		...
		Value &ref_to_value;
		Access(const Key &key, std::map<Key, Value> &all_map, std::mutex &mx) :
				guard_(mx), ref_to_value(all_map[key]) {
		}
	};
	...
	void erase(const Key &key) {
		std::lock_guard < std::mutex
				> guard_(
						vector_value_[static_cast<uint64_t>(key) % bucket_count_].mut);
		vector_value_[static_cast<uint64_t>(key) % bucket_count_].map_val.erase(
				key);
	}
