...
void SearchServer::AddDocument(int document_id, string_view document,
	...
	for (const string &word : words) {
		...
		word_to_document_freqs_[*all_words_.find(word)][document_id] +=
				inv_word_count;
		words_frequency_in_document_[document_id][*all_words_.find(word)] +=
				inv_word_count;
	}
...
std::tuple<std::vector<std::string_view>, DocumentStatus> SearchServer::MatchDocument(
		const std::execution::sequenced_policy&, string_view raw_query,
		int document_id) const {
	return MatchDocument(raw_query, document_id);
}
std::tuple<std::vector<std::string_view>, DocumentStatus> SearchServer::MatchDocument(
		const std::execution::parallel_policy&, string_view raw_query,
		int document_id) const {
	const auto query = ParseQuery(raw_query);

	vector<string_view> matched_words;
	for_each(execution::par, query.plus_words.begin(), query.plus_words.end(),
			[=, &matched_words](const string &word) {
				if (word_to_document_freqs_.at(word).count(document_id)
	...
	for (string_view word : SplitIntoWords(text)) {
		if (!IsValidWord(word)) {
			throw invalid_argument(
					"Word "s + static_cast<std::string>(word) + " is invalid"s);
		}
		if (!IsStopWord(word)) {
...
SearchServer::QueryWord SearchServer::ParseQueryWord(string_view text) const {
	...
	if (word.empty() || word[0] == '-'
			|| !IsValidWord(static_cast<std::string>(word))) {
		throw invalid_argument(
				"Query word "s + static_cast<std::string>(text)
						+ " is invalid"s);
	}
...
void SearchServer::RemoveDocument(const std::execution::parallel_policy&,
	...
	for_each(execution::par,
			words_frequency_in_document_.at(document_id).begin(),
			words_frequency_in_document_.at(document_id).end(),
			[=, &document_id](const auto &a) {
				if (word_to_document_freqs_.at(a.first).count(document_id)
						!= 0) {
