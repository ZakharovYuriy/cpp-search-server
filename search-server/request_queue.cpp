...
std::vector<Document> RequestQueue::AddFindRequest(const std::string& raw_query, DocumentStatus status) {
    QueryResult q;
    q.query = raw_query;
    q.no_empty_query = search_server_.FindTopDocuments(raw_query, status).size();
...
int RequestQueue::GetNoResultRequests() const {
    return std::count_if(requests_.begin(), requests_.end(), [](QueryResult i) {return i.no_empty_query == 0; });
}